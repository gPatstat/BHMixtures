...
posterior_builder=function(f0,Sigma_eps, H, mu_p, Sigma_p){
  ...
  func=function(x){
    ...
    val=-1/2*(t(x-mu_p)%*%inv_Sigma_p%*%(x-mu_p)+t(f0-H%*%p_x)%*%inv_Sigma_eps%*%(f0-H%*%p_x))
...
importance_sampling_p=function(f0,Sigma_eps, H, mu_p, Sigma_p,B=100){
  ...
  for(b in 1:B){
    ...
    wgt[b]=-post_prob(x[b,])+1/2*t(x[b,]-mu_opt)%*%Hess%*%(x[b,]-mu_opt)
  }
return(list(proportions=x,wgt=exp(wgt)))
}
