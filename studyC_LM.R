...
for (par_id in 1:length(m_params)) {
  for (sim_id in 1:nsim) {
    ...
    # F_simulator_lm returns:
    # list(pF=pF_sim, nF, Sigma_eps, coef_noise, basis, D, D0)
    # where pF_sim = list(pF=coefs, basis, D, D0, H_sample, psim=psim$p)
    F_sample <- F_simulator_lm(m = m, n = n, sd_perc = 0.01, mu_p = rep(0, m - 1))
    
    Sigma_eps <- F_sample$Sigma_eps
    H         <- F_sample$pF$H_sample$coefs
    f_coef    <- F_sample$nF
    D         <- F_sample$D
    D0        <- F_sample$D0
    get_basis <- F_sample$basis
    ...
    save(EM_sample, pca_basis, F_sample, get_basis,
         file = paste0("C:/Users/test/OneDrive - Politecnico di Milano/Desktop/BHMixtures/Sim_C_lm/",
                       "simC_LM_sim", sim_id, "par", param_c, "seed", seed, ".rdata"))
    print(sim_id)
  }
}